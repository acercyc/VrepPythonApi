...
def loadApiDummy(clientID):
    """
    Load remoteApiDummy.ttm into the scene to control function calls
    """
...
def callAssociatedScriptFunction(clientID, objName, functionName, operationMode, *args):
    """
    Call a function defined in an associated script
    :param clientID: client ID of this session
    :param objName: The name of the object which this script associated with
    :param functionName: the name of the called function
    :param operationMode: vrep remote API operation mode
    :param args: input arguments. Argements need to be STRING following Lua syntax
    """
    if len(args) > 0:
        argStr = ', ' + ', '.join(args)
    else:
...
def callBuildinFunction(clientID, functionName, operationMode, *args):
    """
    Call Vrep build-in regular API functions
    :param clientID: client ID of this session
    :param functionName: the name of the called function
    :param operationMode: vrep remote API operation mode
    :param args: input arguments. Argements need to be STRING following Lua syntax
...
class VrepController:
    """
    A V-REP controller including wrapper functions to easily control v-rep from python
    """
